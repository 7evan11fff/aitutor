...
console.log('🚀🚀🚀 NEW VERSION LOADED - VERSION 2.0 - WORD COUNT FIXED 🚀🚀🚀');
...
function solveMathProblem(problemText) {
  ...
  if (cleanText.toLowerCase().includes('midpoint') || cleanText.toLowerCase().includes('endpoint') || 
      cleanText.match(/\([^)]*\)/) || cleanText.toLowerCase().includes('coordinate')) {
...
function getDetailedResponse(userInput, context, selectedText = '') {
  ...
  // Enhanced math detection - check for math patterns in the text
  const problemText = selectedText || userInput;
  const hasMathPatterns = problemText.match(/[0-9]+\s*[+\-*/=]\s*[0-9]+/) || 
                         problemText.match(/\d+\.?\d*/) ||
                         problemText.includes('solve') || 
                         problemText.includes('calculate') ||
                         problemText.includes('equation') ||
                         problemText.includes('fraction') ||
                         problemText.includes('algebra') ||
                         problemText.includes('geometry') ||
                         problemText.includes('midpoint') ||
                         problemText.includes('endpoint') ||
                         problemText.includes('coordinate') ||
                         problemText.includes('slope') ||
                         problemText.includes('distance') ||
                         problemText.includes('perimeter') ||
                         problemText.includes('area') ||
                         problemText.includes('volume') ||
                         problemText.includes('angle') ||
                         problemText.includes('triangle') ||
                         problemText.includes('rectangle') ||
                         problemText.includes('circle') ||
                         problemText.includes('square') ||
                         problemText.includes('polygon') ||
                         problemText.match(/\([^)]*\)/) || // Parentheses (coordinates)
  ...
  // Reading responses - but only if it's clearly not math
  if ((context.reading || input.includes('read') || input.includes('text') || input.includes('passage')) && !hasMathPatterns) {
    if (selectedText) {
...
function getReadingAnalysis(text) {
  ...
  return `Let's analyze this text together! I can see you've selected ${wordCount} words in ${sentences} sentences${paragraphs > 1 ? ` across ${paragraphs} paragraphs` : ''}.

**Text Analysis:**

**1. Main Topic:** Based on the key words I see (${uniqueWords.join(', ')}), this text appears to be about ${identifyMainTopic(uniqueWords)}.

**2. First Sentence:** "${firstSentence}"
This often gives us a clue about the main idea. What do you think the author is introducing here?

**3. Key Details to Look For:**
- What specific facts or examples does the author provide?
- Are there any numbers, dates, or statistics mentioned?
- What evidence supports the main points?

**4. Author's Purpose:** 
Look for clues about why this was written:
- Is it trying to inform you about something?
- Is it trying to persuade you to think a certain way?
- Is it telling a story or entertaining?

**5. Questions to Consider:**
- What is the main point the author wants you to understand?
- What details support this main point?
- What can you infer that isn't directly stated?

**Your Turn:** Read through the text again and try to answer these questions. What do you think the main idea is?`;
}
...
function handleNonSelectableClick(e) {
  ...
  // Store the content for analysis (but not for UI elements)
  if (!element.id || !element.id.includes('enhanced-')) {
    selectedText = fullText;
    lastSelectedElement = element;
    console.log('Selected text updated:', selectedText.substring(0, 50) + '...');
  } else {
    console.log('Skipped UI element:', element.id, element.tagName);
  }
